...
def test_ocr_service_uses_paddleocr_for_default_and_preserves_legacy_requests():
    assert REQUESTED_DEFAULT_ENGINE == "paddleocr"
    assert _normalize_engine_name(None) == "paddleocr"
    assert _normalize_engine_name("surya") == "tesseract"
    assert _resolve_engine_name("surya") == "tesseract"
    assert OcrRequest(document_id="doc", pages=["image"]).engine == "paddleocr"
